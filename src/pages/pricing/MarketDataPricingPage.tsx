...
export const MarketDataPricingPage: React.FC<MarketDataPricingPageProps> = ({ onOpenAccount }) => {
  ...
  const marketDataSteps = [
    {
      step: '01',
      title: 'Choose Exchange Feeds',
      desc: 'Select market data from NYSE, NASDAQ, CME Futures, or LSE European equities.',
      icon: Globe
    },
    {
      step: '02',
      title: 'Select Quote Depth',
      desc: 'Pick Level 1 (Top-of-Book Bids/Asks) or Level 2 (Full Orderbook Depth).',
      icon: Layers
    },
    {
      step: '03',
      title: 'Connect Live Stream',
      desc: 'Stream tick-by-tick quotes over sub-10ms WebSockets or direct FIX 4.4 protocol.',
      icon: Wifi
    },
    {
      step: '04',
      title: 'Auto Fee Waiver',
      desc: 'Active trading accounts automatically receive monthly market data fee waivers.',
      icon: CheckCircle2
    }
  ];
  ...
  return (
    <div className="bg-white dark:bg-[#090D14] text-slate-900 dark:text-slate-100 font-sans antialiased transition-colors duration-300 min-h-screen">
      ...
      <section className="relative min-h-[calc(100vh-90px)] flex flex-col justify-center py-6 sm:py-10 overflow-hidden border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0E1420]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center space-y-4 sm:space-y-5 my-auto w-full">
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            >
              <span>Explore Exchange Fees</span>
          ...
          <div className="pt-3 max-w-2xl mx-auto w-full">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-xl bg-slate-50 dark:bg-[#131926] border border-slate-200 dark:border-slate-800 shadow-xs text-center">
              <div>
                <div className="text-xl font-extrabold font-mono text-emerald-600 dark:text-emerald-400">$0 / mo</div>
                <div className="text-xs font-semibold text-slate-600 dark:text-slate-400">Active Trader Waivers</div>
              </div>
              <div className="border-y sm:border-y-0 sm:border-x border-slate-200 dark:border-slate-800 py-2 sm:py-0">
                <div className="text-xl font-extrabold font-mono text-emerald-600 dark:text-emerald-400">Level 1 & 2</div>
                <div className="text-xs font-semibold text-slate-600 dark:text-slate-400">Full Book Depth</div>
              </div>
              <div>
                <div className="text-xl font-extrabold font-mono text-emerald-600 dark:text-emerald-400">&lt; 10 ms</div>
                <div className="text-xs font-semibold text-slate-600 dark:text-slate-400">WebSocket Data Latency</div>
              </div>
            </div>
          </div>
      ...
      {/* 2. CHRONOLOGICAL 4-STEP MARKET DATA WORKFLOW */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <div className="text-center max-w-2xl mx-auto space-y-2">
            <div className="inline-flex items-center gap-1.5 text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">
              ...
              <span>DATA SUBSCRIPTION ROADMAP</span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">
              4 Steps to Stream Live Market Quotes
            </h2>
            <p className="text-slate-600 dark:text-slate-400 text-xs sm:text-sm">
              Connect real-time tick feeds to your WebTrader terminal or quant algorithms.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {marketDataSteps.map((step, idx) => (
      ...
      {/* 3. Primary Market Data Pricing Table */}
      <div id="market-data-pricing" className="py-4">
        <MarketDataPricingSection />
      </div>

      {/* 4. Transparency Cards & Calculator */}
