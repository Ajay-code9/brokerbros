...
import {
  ...
  Check, ExternalLink, Network, FileText, Download, Award, ArrowDown, Radio, Wifi, Zap, Activity as ActivityIcon, MonitorSmartphone
} from 'lucide-react';
...
export const WebSocketsStreamingPage: React.FC<PageProps> = ({ onOpenAccount }) => {
  const [activeFaq, setActiveFaq] = useState<number | null>(null);
  const [activePlan, setActivePlan] = useState<'monthly' | 'annual'>('annual');

  const features = [
    { icon: Radio, title: 'Real-Time Market Data', desc: 'Stream live bid/ask prices, trade ticks, and OHLC candles for thousands of instruments simultaneously over persistent WebSocket connections.', badge: 'Live Feed' },
    { icon: Wifi, title: 'Order Book Streaming (L2)', desc: 'Access full market depth with Level 2 order book updates. See aggregated liquidity across price levels for informed trading decisions.', badge: 'L2 Depth' },
    { icon: Zap, title: 'Sub-10ms Delivery', desc: 'Messages are delivered to connected clients within 10 milliseconds via globally distributed edge servers. Optimized binary encoding minimizes payload size.', badge: 'Ultra-Fast' },
    { icon: Activity, title: 'Account Equity Streams', desc: 'Push real-time account equity, margin level, P&L, and open position updates to your custom front-ends, mobile apps, and dashboards.', badge: 'Account Data' },
    { icon: MonitorSmartphone, title: 'Multi-Device Support', desc: 'Connect from web browsers, mobile apps, desktop terminals, and backend services. Automatic reconnection, heartbeats, and session management built-in.', badge: 'Universal' },
    { icon: ShieldCheck, title: 'Secure WSS Protocol', desc: 'All connections use WSS (WebSocket Secure) with TLS 1.3 encryption, API key authentication, and per-connection rate limiting for enterprise security.', badge: 'Encrypted' }
  ];

  const modules = [
    { icon: Radio, title: 'Price Feed Engine', items: ['Bid/Ask tick streaming', 'OHLC candle generation', 'Instrument subscription', 'Snapshot on connect', 'Historical bar backfill'] },
    { icon: Wifi, title: 'Order Book Service', items: ['L2 market depth', 'Aggregated price levels', 'Incremental updates', 'Full book snapshots', 'Depth of market widgets'] },
    { icon: Activity, title: 'Account Streaming', items: ['Equity real-time push', 'Margin level updates', 'Open P&L streaming', 'Position change events', 'Balance notifications'] },
    { icon: Globe, title: 'Edge Network', items: ['Global CDN distribution', 'Auto-nearest server', 'Connection pooling', 'Load balancing', 'Geographic failover'] },
    { icon: Server, title: 'Pub/Sub Architecture', items: ['Topic-based channels', 'Client group management', 'Message filtering', 'Broadcast capabilities', 'Queue depth monitoring'] },
    { icon: ShieldCheck, title: 'Security & Auth', items: ['WSS/TLS 1.3', 'API key + JWT tokens', 'Per-connection rate limits', 'IP whitelist support', 'Connection audit logs'] }
  ];

  const integrations = [
    { name: 'MetaTrader 5', desc: 'Price Feed' },
    { name: 'cTrader', desc: 'Price Feed' },
    { name: 'FIX Protocol', desc: 'Market Data' },
    { name: 'Socket.io', desc: 'Client Library' },
    { name: 'Redis Pub/Sub', desc: 'Message Bus' },
    { name: 'CloudFlare', desc: 'Edge CDN' },
    { name: 'Akamai', desc: 'Edge CDN' },
    { name: 'Kafka', desc: 'Event Streaming' },
    { name: 'React', desc: 'Front-End SDK' },
    { name: 'Flutter', desc: 'Mobile SDK' }
  ];

  const testimonials = [
    { quote: "We replaced our polling-based price feed with BrokerBros WebSockets and saw latency drop from 500ms to under 10ms. Our traders noticed the difference immediately.", name: "Tom Bergman", role: "CTO, Nordic Trading Solutions", rating: 5 },
    { quote: "The L2 order book streaming powers our custom trading interface. We can show real-time market depth to 50,000 concurrent users without any performance issues.", name: "Maria Gonzalez", role: "VP Engineering, LatAm Markets", rating: 5 },
    { quote: "Account equity streaming is seamless. Our mobile app shows real-time P&L and margin levels without any polling. The automatic reconnection logic handles network drops gracefully.", name: "Kevin Park", role: "Mobile Lead, K-Trade", rating: 5 }
  ];

  const faqs = [
    { q: 'How many concurrent connections are supported?', a: 'Our infrastructure supports 100,000+ concurrent WebSocket connections per cluster. Enterprise plans include dedicated clusters that can scale to millions of connections.' },
    { q: 'What is the message delivery latency?', a: 'Messages are delivered to connected clients within sub-10ms latency via globally distributed edge servers. We use optimized binary encoding (MessagePack) to minimize payload size.' },
    { q: 'Do you support order book (L2) data?', a: 'Yes. Full Level 2 market depth with aggregated price levels. Clients receive incremental updates and can request full snapshots on connect.' },
    { q: 'Is there auto-reconnection logic?', a: 'Yes. Our client SDKs include automatic reconnection with exponential backoff, heartbeat monitoring, and session resumption to ensure zero data loss during network interruptions.' },
    { q: 'What client libraries are available?', a: 'We provide official SDKs for JavaScript/React, Flutter/Dart, Swift (iOS), Kotlin (Android), Python, and C#. Each includes full TypeScript definitions and code samples.' },
    { q: 'Can I stream custom data channels?', a: 'Yes. Beyond price and account data, you can create custom pub/sub channels for alerts, news feeds, or any real-time data your application requires.' }
  ];

  const plans = [
    { name: 'Starter Stream', price: activePlan === 'annual' ? '$199' : '$249', period: '/ month', desc: 'Basic streaming for small projects.', features: ['1,000 Concurrent Connections', 'Price Feed Only', 'JavaScript SDK', 'Standard Support', 'Sandbox Access'], cta: 'Request Demo', highlight: false },
    { name: 'Pro Stream', price: activePlan === 'annual' ? '$599' : '$749', period: '/ month', desc: 'Full streaming infrastructure.', features: ['25,000 Concurrent Connections', 'L2 Order Book', 'Account Equity Streams', 'All Client SDKs', 'Global Edge CDN', '24/7 Priority Support'], cta: 'Start Free Trial', highlight: true },
    { name: 'Enterprise', price: 'Custom', period: '', desc: 'Dedicated streaming cluster.', features: ['Unlimited Connections', 'Custom Data Channels', 'Dedicated Cluster', 'SLA Guarantee', 'Dedicated Engineer'], cta: 'Contact Sales', highlight: false }
  ];
  ...
  return (
    <div className="bg-white dark:bg-[#0a0a0a] text-slate-900 dark:text-slate-300 min-h-screen transition-colors duration-300 selection:bg-emerald-500/30 font-sans">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="max-w-2xl">
              ...
              <p className="text-lg text-slate-600 dark:text-slate-400 mb-8 leading-relaxed font-light">
                Deliver real-time market data, order book updates, and account equity streams directly to your custom front-ends and mobile apps.
              </p>
      ...
      <ProductVideoSection
        title="Ultra-Low Latency WebSockets Streaming Tour"
        subtitle="Watch live streaming of L2 order books, real-time bid/ask tickers, and instant account equity updates across global edge servers."
        videoSrc="/test2.mp4"
      ...
      <ProductGallerySection
        title="WebSockets Streaming & Market Data Suite Gallery"
        subtitle="View real-time L2 order book widgets, tick-by-tick streaming monitors, and edge server connection diagnostics."
      />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 divide-x divide-slate-200 dark:divide-slate-800/50">
            <div className="text-center">
              <div className="text-3xl font-black text-slate-900 dark:text-white font-mono mb-1">100k+</div>
              <div className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Msg / Sec</div>
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">CDN Edge</div>
            </div>
            <div className="text-center">
      ...
      <section className="py-24 border-b border-slate-200 dark:border-slate-800/50 bg-slate-50/80 dark:bg-transparent relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white mb-4">Streaming Infrastructure</h2>
            <p className="text-slate-600 dark:text-slate-400 text-lg">Deliver real-time market data, order book depth, and account updates to any device with ultra-low latency WebSocket connections.</p>
          </div>
      ...
      <section className="py-24 border-b border-slate-200 dark:border-slate-800/50 bg-white dark:bg-[#0a0a0a]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-center mb-16">
            <div>
              <h2 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white mb-4">Streaming Architecture</h2>
              <p className="text-slate-600 dark:text-slate-400 text-lg">Globally distributed pub/sub infrastructure purpose-built for high-frequency financial data delivery with enterprise-grade reliability.</p>
            </div>
      ...
      <section className="py-24 relative overflow-hidden bg-slate-50/80 dark:bg-transparent">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <h2 className="text-4xl sm:text-5xl font-black text-slate-900 dark:text-white mb-6">Scale Your WebSockets Infrastructure</h2>
