        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {stats.map((s, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-white p-6 rounded-2xl border border-slate-200 text-center space-y-3 hover:shadow-xs transition-all group"
              >
                <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center justify-center mx-auto group-hover:scale-105 transition-transform">
                  <IconComp className="w-5 h-5" />
                </div>
                <div className="text-2xl sm:text-3xl font-black font-mono text-emerald-700 tracking-tight">
                  {s.value}
                </div>
                <div className="text-xs font-bold text-slate-900 font-sans">{s.label}</div>
                <p className="text-[11px] text-slate-500 leading-normal font-sans">{s.subtext}</p>
              </div>
            );
          })}
        </div>
