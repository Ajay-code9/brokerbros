...
export const FeeTransparencyCards: React.FC = () => {
  ...
  return (
    <section className="py-6 sm:py-9 bg-[#0B1722] border-b border-emerald-900/30 text-white font-sans relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 relative z-10 font-sans">
        ...
        <div className="text-center max-w-2xl mx-auto space-y-1">
          <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-[11px] font-mono font-bold uppercase tracking-wider">
          ...
          <p className="text-slate-300 text-xs sm:text-sm leading-relaxed font-sans max-w-lg mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-5xl mx-auto">
          {commitments.map((item, idx) => (
            <div
              key={idx}
              className="bg-slate-900/90 rounded-xl p-4 sm:p-5 border border-slate-800 hover:border-emerald-500/40 transition-all duration-200 flex flex-col justify-between space-y-3"
            >
              <div className="space-y-2">
                <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                  <div>
                    <h3 className="text-sm sm:text-base font-bold text-white font-sans">{item.title}</h3>
                    <div className="text-[10px] text-emerald-400 font-mono mt-0.5">{item.subText}</div>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-lg font-black font-mono text-emerald-400">{item.metric}</div>
                  </div>
                </div>
              ...
              <div className="pt-2 border-t border-slate-800/80 space-y-1 text-xs font-sans">
                {item.bullets.map((bullet, bIdx) => (
                  <div key={bIdx} className="flex items-center gap-2 text-slate-300 text-[11px]">
        ...
        <div className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 max-w-5xl mx-auto text-center sm:text-left font-sans">
          <div className="flex items-center space-x-2.5">
            <div className="w-7 h-7 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center shrink-0">
              <FileText className="w-3.5 h-3.5 text-emerald-400" />
            </div>
            <div>
              <h4 className="text-xs font-bold text-white">Audited SEC & FINRA Disclosure Schedule</h4>
              <p className="text-[11px] text-slate-400">Itemized 10-page fee schedule updated quarterly and published publicly.</p>
            </div>
          </div>
