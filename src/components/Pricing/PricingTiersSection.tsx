...
export const PricingTiersSection: React.FC<{ onOpenAccount: () => void }> = ({ onOpenAccount }) => {
  ...
  return (
    <section className="bg-slate-50/70 py-8 lg:py-12 border-b border-slate-200 relative overflow-hidden font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Header - Compact & Tight */}
        <div className="text-center max-w-3xl mx-auto space-y-2 mb-6">
          <div className="inline-flex items-center space-x-1.5 px-3 py-0.5 rounded-full bg-white border border-slate-200 text-slate-700 text-[11px] font-mono font-semibold shadow-2xs">
          ...
          <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-950 tracking-tight leading-tight">
            Simple Flat Monthly SaaS Tiers. <span className="text-emerald-600">Zero Commission Markup.</span>
          </h2>
          ...
          <div className="flex items-center justify-center space-x-3 pt-2">
            ...
            <button
              ...
              className="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-slate-300 p-0.5 transition-colors duration-200 ease-in-out focus:outline-none"
            >
              <span className="sr-only">Toggle annual billing</span>
              <span
                className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-emerald-600 shadow-sm transition duration-200 ease-in-out ${
                  isAnnual ? 'translate-x-5' : 'translate-x-0'
                }`}
              />
            </button>

            <span className={`text-xs font-semibold flex items-center space-x-1 ${isAnnual ? 'text-slate-900' : 'text-slate-500'}`}>
            ...
            return (
              <div
                ...
                className={`rounded-2xl p-5 flex flex-col justify-between transition-all duration-300 relative ${
                  tier.popular
                    ? 'bg-white border-2 border-emerald-600 shadow-lg shadow-emerald-600/10 z-20'
                    : 'bg-white border border-slate-200 shadow-2xs hover:border-slate-300'
                }`}
              >
                {tier.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-emerald-600 text-white text-[10px] font-bold px-3 py-0.5 rounded-full uppercase tracking-wider shadow-sm">
                    {tier.badge}
                  </div>
                )}
                ...
                <div>
                  <div className="mb-3">
                    <h3 className="text-lg font-extrabold text-slate-900 font-sans">{tier.name}</h3>
                  ...
                  <div className="mb-4 pb-4 border-b border-slate-100">
                    {isCustom ? (
                      <div className="text-2xl font-extrabold text-slate-900 font-sans">Custom Tier</div>
                    ) : (
                  ...
                  <div className="space-y-2 mb-5">
                    <p className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-wider">What's Included:</p>
                    {tier.features.map((feat, i) => (
                      <div key={i} className="flex items-start space-x-2 text-xs text-slate-700">
                        <div className="w-4 h-4 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center shrink-0 mt-0.5 border border-emerald-200/60">
