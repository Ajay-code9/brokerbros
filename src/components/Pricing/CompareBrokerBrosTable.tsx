...
export const CompareBrokerBrosTable: React.FC<{ onOpenAccount: () => void }> = ({ onOpenAccount }) => {
  ...
  return (
    <section className="py-12 sm:py-16 bg-slate-50/80 dark:bg-[#0E1420] border-b border-slate-200 dark:border-[#1E293B] text-slate-900 dark:text-white font-sans relative overflow-hidden transition-colors duration-300">
      
      {/* Background Subtle Accent */}
      <div className="absolute top-0 right-1/4 w-[500px] h-[500px] bg-emerald-100/40 dark:bg-emerald-500/10 rounded-full blur-[140px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10 relative z-10 font-sans">
        ...
        <div className="overflow-x-auto bg-white dark:bg-[#141C2B] rounded-2xl border border-slate-200 dark:border-[#1E293B] shadow-xl">
          <table className="w-full text-left text-xs font-sans min-w-[700px]">
            <thead>
              <tr className="bg-slate-950 dark:bg-[#090D14] text-white font-mono uppercase tracking-wider text-[11px] border-b border-slate-800 dark:border-[#1E293B]">
                <th className="py-4 px-5 font-bold text-white w-1/4">Feature / Pricing Dimension</th>
                <th className="py-4 px-5 text-emerald-400 font-bold bg-slate-900 dark:bg-[#1B2538] border-x-2 border-emerald-500 shadow-inner">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-[#1E293B] text-slate-700 dark:text-slate-300">
              {features.map((f, idx) => (
                <tr key={idx} className={idx % 2 === 0 ? 'bg-white dark:bg-[#141C2B] hover:bg-slate-50/80 dark:hover:bg-[#1B2538] transition-colors' : 'bg-slate-50/40 dark:bg-[#0E1420]/60 hover:bg-slate-50/90 dark:hover:bg-[#1B2538] transition-colors'}>
                  <td className="py-3.5 px-5 font-bold text-slate-900 dark:text-white font-sans">
                    {f.label}
                  </td>
                  <td className="py-3.5 px-5 font-mono font-extrabold text-emerald-700 dark:text-emerald-400 bg-emerald-50/60 dark:bg-emerald-500/10 border-x-2 border-emerald-500/80">
                    {f.brokerbros}
                  </td>
                  <td className="py-3.5 px-4 font-mono text-slate-600 dark:text-slate-300">{f.schwab}</td>
                  <td className="py-3.5 px-4 font-mono text-slate-600 dark:text-slate-300">{f.etrade}</td>
                  <td className="py-3.5 px-4 font-mono text-slate-600 dark:text-slate-300">{f.fidelity}</td>
                  <td className="py-3.5 px-4 font-mono text-slate-600 dark:text-slate-300">{f.neo}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {/* Action Banner */}
        <div className="bg-white dark:bg-[#141C2B] p-6 rounded-2xl border border-slate-200 dark:border-[#1E293B] shadow-md flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="space-y-1 text-center sm:text-left">
            <div className="text-base font-bold text-slate-900 dark:text-white">Ready to stop overpaying legacy brokers?</div>
            <div className="text-xs text-slate-600 dark:text-slate-300">Transfer existing portfolios seamlessly with up to $150 in ACATS fee reimbursements.</div>
          </div>
          <button
